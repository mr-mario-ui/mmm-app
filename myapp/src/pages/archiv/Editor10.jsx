...
export default function Editor() {
  ...
  const edges = visibleNodes.map(node => {
    ...
    return (
      <path key={`e-${node.id}`}
        d={`M ${startX} ${p.y} H ${midX} V ${node.y} H ${endX}`}
        fill="none" stroke={color} strokeWidth="1.4" strokeOpacity="0.4"
      ...
      return (
        <g key={node.id}
          onMouseDown={e => { e.stopPropagation(); dragId.current=node.id; didDrag.current=false
            dragOff.current={x:(e.clientX-pan.x)/zoom - node.x, y:(e.clientY-pan.y)/zoom - node.y}
    ...
    <div style={{ width:'100vw', height:'100vh', display:'flex', flexDirection:'column',
      ...
      onMouseMove={e => {
        if (dragId.current) {
          ...
          const nx = (e.clientX - pan.x) / zoom - dragOff.current.x
          const ny = (e.clientY - pan.y) / zoom - dragOff.current.y
