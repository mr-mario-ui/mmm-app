...
export default function Editor() {
  ...
  // Canvas drag — refs bleiben immer aktuell, keine stale closures
  ...
  const renderNode = (node) => {
    ...
    const isDrg    = node.id === dragId.current
      ...
      return (
        <g key={node.id} style={{ opacity: isDrg ? 0.7 : 1 }}>
          ...
          <circle cx={node.x} cy={node.y} r={R}
            ...
            onMouseDown={e => { e.stopPropagation(); dragId.current=node.id; didDrag.current=false; dragOff.current={x:(e.clientX-panRef.current.x)/zoomRef.current - node.x, y:(e.clientY-panRef.current.y)/zoomRef.current - node.y}; setSelectedId(node.id) }}
  ...
  return (
    <div style={{ width:'100vw', height:'100vh', display:'flex', flexDirection:'column',
      ...
      onMouseMove={e => {
        ...
        if (isResizingList.current) setListWidth(Math.max(200, Math.min(600, resizeListW.current + (resizeListX.current - e.clientX))))
        if (isResizingComment.current) setCommentHeight(Math.max(80, Math.min(500, resizeCommentH.current + (resizeCommentY.current - e.clientY))))
      }}
      onMouseUp={async () => {
        ...
        isPan.current = false; isResizingList.current = false; isResizingComment.current = false
        document.body.style.cursor = ''
      }}>
