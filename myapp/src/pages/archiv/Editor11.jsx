...
export default function Editor() {
  ...
  const edges = visibleNodes.map(node => {
    ...
    const pW     = pDepth===0 ? 80 : Math.min(Math.max((p.label.length>24?23:p.label.length)*( pDepth===1?12:11)*0.57+20, 56), 175)
    const startX = p.x + pW/2
    // Child left edge
    const cDepth = getDepth(node, nodes)
    const cLbl   = node.label.length>24 ? node.label.slice(0,23)+'…' : node.label
    const cW     = Math.min(Math.max(cLbl.length*(cDepth===1?12:11)*0.57+20, 56), 175)
    const endX   = node.x - cW/2
    const midX   = (startX + endX) / 2
    const bend = Math.abs(endX - startX) * 0.55
    return (
      <path key={`e-${node.id}`}
        d={`M ${startX} ${p.y} C ${startX+bend} ${p.y}, ${endX-bend} ${node.y}, ${endX} ${node.y}`}
  ...
  const nodeEls = visibleNodes.map(node => {
    ...
    if (depth === 0) {
      ...
      return (
        <g key={node.id}
          onMouseDown={e => { e.stopPropagation(); dragId.current=node.id; didDrag.current=false
            dragOff.current={x:(e.clientX-panRef.current.x)/zoomRef.current - node.x, y:(e.clientY-panRef.current.y)/zoomRef.current - node.y}
            setSelectedId(node.id) }}
          onDoubleClick={e => { e.stopPropagation(); openRename(node) }}
          style={{ cursor:'grab' }}>
          {isSel && <rect x={node.x-RW/2-6} y={node.y-RH/2-6} width={RW+12} height={RH+12} rx={RH/2+6}
            fill="none" stroke="#3b82f6" strokeWidth="1.5" strokeOpacity="0.4" strokeDasharray="4 3"/>}
          <rect x={node.x-RW/2} y={node.y-RH/2} width={RW} height={RH} rx={RH/2}
            fill={isSel?'#1e293b':'white'}
            stroke={isSel?'#1e293b':'#d1d5db'} strokeWidth={isSel?0:1.5} filter="url(#sh)"/>
          <text x={node.x} y={node.y+5} textAnchor="middle"
            fill={isSel?'white':'#111827'} fontSize="13" fontWeight="800"
    ...
    return (
      <g key={node.id}
        ...
        onDoubleClick={e => { e.stopPropagation(); openRename(node) }}
        style={{ cursor:'grab' }}>
        {/* Selektion: farbiger Ring außen */}
        {isSel && <rect x={node.x-W/2-5} y={node.y-H/2-5} width={W+10} height={H+10} rx={rx+5}
          fill={color+'18'} stroke={color} strokeWidth="1.5" strokeOpacity="0.6"/>}
        <rect x={node.x-W/2} y={node.y-H/2} width={W} height={H} rx={rx}
          fill={isSel ? color : depth===1 ? color+'18' : 'white'}
          stroke={isSel ? color : depth===1 ? color+'70' : '#e5e7eb'}
          strokeWidth={isSel?0:1}
          filter={depth<=2?'url(#sh)':'none'}/>
        ...
        {/* Collapse */}
        {hasKids && (
          <g onMouseDown={e=>e.stopPropagation()} onClick={e=>{e.stopPropagation();toggleCollapse(node.id)}} style={{cursor:'pointer'}}>
        ...
        {node.comment && <circle cx={node.x-W/2-7} cy={node.y} r={3} fill={color} fillOpacity="0.5"/>}
      </g>
    )
  })
  ...
  return (
    <div style={{ width:'100vw', height:'100vh', display:'flex', flexDirection:'column',
      ...
      onMouseUp={async e => {
        if (dragId.current && didDrag.current) {
          const id = dragId.current
          const node = nodes.find(n => n.id===id)
          if (node) await supabase.from('nodes').update({x:node.x, y:node.y}).eq('id', id)
        }
        dragId.current=null; didDrag.current=false
        isPan.current=false; isResL.current=false; isResC.current=false; document.body.style.cursor=''
      }}>
      ...
      {/* Header: 3-Spalten — Titel links | Toolbar Mitte | Status rechts */}
      <div style={{ height:46, borderBottom:'1px solid #e5e7eb', display:'grid',
        gridTemplateColumns:'1fr auto 1fr', alignItems:'center',
        padding:'0 8px', flexShrink:0, background:'white', zIndex:10 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:2,
          background:'#f9fafb', border:'1px solid #e5e7eb', borderRadius:10, padding:'3px 6px' }}>
          ...
          {selectedNode && (<>
            ...
            <span style={{ fontSize:10, color:'#9ca3af', fontFamily:'monospace', padding:'0 2px' }}>
              {getOutlineNumber(selectedNode,nodes)||'●'}
            </span>
            <span style={{ fontSize:11, fontWeight:600, color:'#374151', maxWidth:100,
        ...
        <div style={{ display:'flex', justifyContent:'flex-end', alignItems:'center' }}>
          <span style={{ fontSize:11, color:saved?'#22c55e':'#f59e0b', fontWeight:600 }}>
            {saved?'✓ gespeichert':'⟳ …'}
          </span>
        </div>
      </div>
      ...
      <div style={{ flex:1, display:'flex', overflow:'hidden', minHeight:0 }}>
        ...
        <div style={{ flex:1, position:'relative', overflow:'hidden' }}>
          <svg ref={svgRef} style={{ width:'100%', height:'100%', background:'#f9fafb', display:'block' }}
            ...
            onWheel={e => { e.preventDefault(); setZoom(z=>{ const nz=Math.min(2.5,Math.max(0.25,z-e.deltaY*0.001)); zoomRef.current=nz; return nz }) }}>
            <defs>
        ...
        {/* Resize handle */}
        <div onMouseDown={e=>{isResL.current=true;resLx.current=e.clientX;resLw.current=listW;document.body.style.cursor='col-resize';e.preventDefault()}}
