...
// ── Rich Text Editor (Quill) - StrictMode-sicher ─────────────
function RichTextEditor({ value, onChange, accentColor }) {
  const quillRef   = useRef(null)
  const isInternal = useRef(false)
  const mountedRef = useRef(false)

  // ref-callback: wird nur beim echten ersten Mount aufgerufen
  const containerCb = useCallback((el) => {
    ...
    if (window.Quill) {
      ...
      const existing = document.getElementById('quill-script')
      if (existing) {
        // Warten bis geladen
        const t = setInterval(() => { if (window.Quill) { clearInterval(t); doInit() } }, 50)
      } else {
  ...
  // Wert von außen aktualisieren wenn Knoten wechselt
  useEffect(() => {
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minHeight:0 }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden',
        ...
        <div ref={containerCb} style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}/>
      </div>
    </div>
  )
}
...
export default function Editor() {
  ...
  const renderBranchEdges = (branchNode) => {
    ...
    // stemX = fixer x-Wert wo die vertikale Linie sitzt (= x-Position der Kinder)
    const renderSubEdges = (parentNode, parentDepth) => {
      const children = visibleNodes.filter(n => n.parent_id === parentNode.id)
      if (!children.length) return

      // Die Kinder stehen alle auf derselben x-Spalte (vom Layout gesetzt)
      // stemX liegt direkt an der x-Position der Kinder
      const childX = children[0].x  // alle Kinder gleiche x-Spalte
      // Kleine horizontale Verbindung: stemX liegt zwischen Parent und Kindern
      const stemX = childX  // Sammellinie genau an der x-Position der Kinder

      const topY    = children[0].y
      const bottomY = children[children.length - 1].y

      // Horizontale Verbindung vom Elternrand zur Sammellinie
      // Elternrand
      let parentEdgeX
      if (parentDepth === 1) {
        parentEdgeX = isRight ? parentNode.x + mainW : parentNode.x - mainW
      } else {
        // Textknoten: x ist der Textstart (links/rechts-ausgerichtet)
        // Auf der rechten Seite: x = linker Startpunkt des Textes → rechtes Ende = x + tw
        // Auf der linken Seite: x = rechter Endpunkt des Textes → linkes Ende = x - tw
        // Aber wir wollen den Punkt wo die Linie abgeht: Ende des Textes
        const fs = 12; const tw = parentNode.label.length * fs * 0.55 + 4
        parentEdgeX = isRight ? parentNode.x + tw : parentNode.x - tw
      }

      // Horizontale Linie vom Elternrand zum stemX-Knick
      if (Math.abs(parentEdgeX - stemX) > 2) {
        const knickY = parentNode.y
        elements.push(
          <line key={`knick-${parentNode.id}`}
            x1={parentEdgeX} y1={knickY}
            x2={stemX} y2={knickY}
            stroke={color} strokeWidth={parentDepth === 1 ? 1.2 : 0.8}
      ...
      // Horizontale Abzweiger von Sammellinie zu jedem Kind-Text
      children.forEach(child => {
        const fs = 12; const tw = child.label.length * fs * 0.55 + 4
        const textStartX = isRight ? child.x : child.x - tw

        elements.push(
          <line key={`h-${child.id}`}
            x1={stemX} y1={child.y}
            x2={isRight ? child.x - 3 : child.x + 3} y2={child.y}
            stroke={color} strokeWidth={parentDepth === 1 ? 1.0 : 0.7}
            strokeOpacity="0.4" strokeLinecap="round"/>
        )

        renderSubEdges(child, parentDepth + 1)
      })
    }
